/* =====================================================================
   ...
   Doppelte Mitlaute (mm, nn, ll, tt, pp, rr, ff, ss) als PDF -> identisch iOS/Android.
   Vier Aufgabentypen:
...
(function (global) {
  ...
  function drawHeader(ctx, opts, sub) {
    ...
    ctx.text('Doppelte Mitlaute', PT.marginX, top, { font: F.heavy, size: 13, color: C.red2 });
  ...
  const RULE = [{ t: 'Nach einem' }, { t: 'kurzen, betonten Selbstlaut', b: 1 }, { t: 'wird der Mitlaut oft' }, { t: 'verdoppelt', b: 1 }, { t: '(So' }, { t: 'mm', b: 1 }, { t: 'er, Ba' }, { t: 'll', b: 1 }, { t: ', Mu' }, { t: 'tt', b: 1 }, { t: 'er). Nach einem' }, { t: 'langen', b: 1 }, { t: 'Selbstlaut bleibt er' }, { t: 'einfach', b: 1 }, { t: '(T\u00fcte, malen, Schal).' }];
  ...
  async function buildWorksheetPDF(spec, opts, _unused) {
    ...
    function renderTaskSet(setTasks){
      ...
      if (groups.fix.length) { nr = renderSort(groups.fix, nr); }
    }
    ...
    if (showSol) {
      newPage('L\u00f6sung \u2013 Doppelte Mitlaute');
      const allTasks = [];
      ...
      if (groups.fix.length) { snr = solSort(groups.fix, snr); }
    }
    ...
    function renderGap(arr, startNr) {
      y = secHead(ctx, 'Erg\u00e4nze die fehlenden Mitlaute.', y);
      const colW = W / 2;
      const rowH = fs * 1.7;
    ...
    function renderSort(arr, startNr) {
      arr.forEach(t => {
        ...
        const bank = t.words.map(o => isFix ? o.disp : o.w);
        const tableRows = Math.ceil(t.words.length / nc) + 1;
        const blockH = 9 * 1.5 + 16 * MM + 7 * MM + tableRows * 8 * MM + 8 * MM;
        ensure(blockH);
        const head = isFix
          ? 'Manche W\u00f6rter sind falsch geschrieben. Schreibe sie richtig in die passende Spalte.'
          : 'Sprich die W\u00f6rter und ordne sie ein: kurzer Selbstlaut -> doppelt, langer -> einfach.';
    ...
    function solSort(arr, startNr) {
      ...
      arr.forEach(t => {
        ...
        y += bodyH + 2 * MM;
        if (isFix) { ctx.text('Unterstrichene W\u00f6rter waren falsch geschrieben.', PT.marginX, y, { font: fonts.regular, size: 7.5, color: C.gray }); y += 4 * MM; }
        y += 2 * MM;
      });
      return startNr + arr.length;
    }
  ...
  global.DoppelkonsPDF = { PT, buildWorksheetPDF };
