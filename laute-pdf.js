/* =====================================================================
   laute-pdf.js  ·  PDF-Modul (pdf-lib)
   © 2026 Sascha Rothenburg

   Ähnlich klingende Laute (ä/e, äu/eu, v/f, chs/x/ks, pf/f, s/ss/ß) als PDF -> identisch iOS/Android.
   Vier Aufgabentypen:
...
(function (global) {
  ...
  function drawHeader(ctx, opts, sub) {
    ...
    ctx.text('\u00c4hnlich klingende Laute', PT.marginX, top, { font: F.heavy, size: 13, color: C.red2 });
  ...
  const RULES = {
    ae: [{ t: 'H\u00f6rst du ein \u00e4? Suche das verwandte Wort mit' }, { t: 'a', b: 1 }, { t: ': B' }, { t: '\u00e4', b: 1 }, { t: 'cker \u2013 b' }, { t: 'a', b: 1 }, { t: 'cken, H' }, { t: '\u00e4', b: 1 }, { t: 'nde \u2013 H' }, { t: 'a', b: 1 }, { t: 'nd. Dann schreibst du' }, { t: '\u00e4', b: 1 }, { t: ', nicht e.' }],
    aeu: [{ t: 'Bei' }, { t: '\u00e4u', b: 1 }, { t: 'hilft das verwandte Wort mit' }, { t: 'au', b: 1 }, { t: ': l' }, { t: '\u00e4u', b: 1 }, { t: 'ft \u2013 l' }, { t: 'au', b: 1 }, { t: 'fen, B' }, { t: '\u00e4u', b: 1 }, { t: 'me \u2013 B' }, { t: 'au', b: 1 }, { t: 'm. Dann schreibst du' }, { t: '\u00e4u', b: 1 }, { t: ', nicht eu.' }],
    vf: [{ t: 'Manche W\u00f6rter klingen wie' }, { t: 'f', b: 1 }, { t: ', werden aber mit' }, { t: 'v', b: 1 }, { t: 'geschrieben. Das sind Merkw\u00f6rter:' }, { t: 'V', b: 1 }, { t: 'ogel,' }, { t: 'V', b: 1 }, { t: 'ater,' }, { t: 'v', b: 1 }, { t: 'iel,' }, { t: 'v', b: 1 }, { t: 'ier.' }],
    chs: [{ t: 'Den Laut' }, { t: 'x', b: 1 }, { t: 'kann man verschieden schreiben:' }, { t: 'chs', b: 1 }, { t: '(Fu' }, { t: 'chs', b: 1 }, { t: '),' }, { t: 'x', b: 1 }, { t: '(He' }, { t: 'x', b: 1 }, { t: 'e) oder' }, { t: 'ks', b: 1 }, { t: '(lin' }, { t: 'ks', b: 1 }, { t: '). Das sind Merkw\u00f6rter.' }],
    pf: [{ t: 'Sprich deutlich: bei' }, { t: 'pf', b: 1 }, { t: 'h\u00f6rst du beide Laute \u2013' }, { t: 'Pf', b: 1 }, { t: 'erd, A' }, { t: 'pf', b: 1 }, { t: 'el, Ko' }, { t: 'pf', b: 1 }, { t: '. Sonst steht nur' }, { t: 'f', b: 1 }, { t: '.' }],
    sss: [{ t: 's', b: 1 }, { t: 'klingt weich (Ro' }, { t: 's', b: 1 }, { t: 'e),' }, { t: 'ss', b: 1 }, { t: 'und' }, { t: '\u00df', b: 1 }, { t: 'klingen scharf (Flu' }, { t: 'ss', b: 1 }, { t: ', Fu' }, { t: '\u00df', b: 1 }, { t: '). Nach kurzem Vokal steht' }, { t: 'ss', b: 1 }, { t: ', nach langem Vokal' }, { t: '\u00df', b: 1 }, { t: '.' }],
  };
  ...
  async function buildWorksheetPDF(spec, opts, _unused) {
    ...
    function renderRuleBoxes(setTasks){
      ...
      ['ae', 'aeu', 'vf', 'chs', 'pf', 'sss'].forEach(ph => {
    ...
    if (showSol) {
      newPage('L\u00f6sung \u2013 \u00c4hnlich klingende Laute');
      const allTasks = [];
    ...
    function renderGap(arr, startNr) {
      y = secHead(ctx, 'Setze die richtigen Buchstaben ein.', y);
      const colW = W / 2;
      const rowH = fs * 1.7;
      for (let i = 0; i < arr.length; i += 2) {
        ...
        for (let c = 0; c < 2 && i + c < arr.length; c++) {
          ...
          // Lücke: Breite passt zur Buchstabenanzahl (ä, äu, chs, ss ...)
          const blankW = Math.max(10, g.gap.length * 7 + 4);
    ...
    function renderDerive(arr, startNr) {
      y = secHead(ctx, 'Finde das verwandte Wort und schreibe das gesuchte Wort richtig auf.', y);
      const colW = W / 2;
      const rowH = inlineRowH(style, fs);
      // Feste Sub-Spalten: Nummern, Lückenwörter, Pfeile und Linien stehen bündig untereinander
      const nrW = 16;
      let wordW = 0;
      arr.forEach(t => {
        const g = gapInfo(t.it.raw);
        const bw = Math.max(10, g.gap.length * 7 + 4);
        const ww = ctx.textWidth(g.pre, fonts.bold, fs) + 2 + bw + 2 + ctx.textWidth(g.post, fonts.bold, fs);
        wordW = Math.max(wordW, ww);
      });
      const arrowW = ctx.textWidth('->', fonts.regular, fs);
      const gap = 6;
      const lineStart = nrW + wordW + gap + arrowW + gap;
      const lineW = Math.min(120, colW - lineStart - 6 * MM);
      for (let i = 0; i < arr.length; i += 2) {
        ensure(rowH);
        for (let c = 0; c < 2 && i + c < arr.length; c++) {
          const t = arr[i + c];
          const g = gapInfo(t.it.raw);
          const x = PT.marginX + c * colW;
          const baseY = inlineBaselineY(y, fs, style);
          const nrStr = (startNr + i + c) + '.';
          ctx.textBaseline(nrStr, x, baseY, { font: fonts.bold, size: 9, color: C.gray });
          let tx = x + nrW;
          ctx.textBaseline(g.pre, tx, baseY, { font: fonts.bold, size: fs, color: C.ink });
    ...
    function renderSort(arr, startNr) {
      arr.forEach(t => {
        ...
        y = secHead(ctx, 'Ordne die W\u00f6rter in die richtige Spalte (' + c.labelA + ' oder ' + c.labelB + ').', y);
        // Wortvorrat-Box
        const wfs = fs;
        ...
        ctx.textCentered(c.labelA, PT.marginX + colW / 2, y + (headH - 11) / 2, { font: fonts.heavy, size: 11, color: C.red2 });
        ctx.textCentered(c.labelB, PT.marginX + colW * 1.5, y + (headH - 11) / 2, { font: fonts.heavy, size: 11, color: C.red2 });
        y += headH;
        const cellH = 8 * MM;
    ...
    function solGap(arr, startNr) {
      ...
      for (let i = 0; i < arr.length; i += 2) {
        ...
        for (let c = 0; c < 2 && i + c < arr.length; c++) {
          ...
          if (g.deriv && g.deriv !== '---') ctx.text('(' + g.deriv + ')', tx, y + 1.5, { font: fonts.regular, size: 8, color: C.gray });
        }
        y += rowH;
      }
      y += 2 * MM;
    ...
    function solDerive(arr, startNr) {
      ...
      for (let i = 0; i < arr.length; i += 2) {
        ...
        for (let c = 0; c < 2 && i + c < arr.length; c++) {
          ...
          ctx.text(nrStr, x, y + 1, { font: fonts.bold, size: 9, color: C.gray });
          let tx = x + ctx.textWidth(nrStr, fonts.bold, 9) + 4;
          ctx.text(t.it.grund, tx, y, { font: fonts.bold, size: fs, color: C.red2 });
          tx += ctx.textWidth(t.it.grund, fonts.bold, fs) + 5;
          ctx.text('(von ' + t.it.deriv + ')', tx, y + 1.5, { font: fonts.regular, size: 8, color: C.gray });
        }
        y += rowH;
      }
      y += 2 * MM;
    ...
    function solSort(arr, startNr) {
      ...
      arr.forEach(t => {
        ...
        ctx.text(c.labelA, PT.marginX + 3 * MM, y + (headH - 9) / 2, { font: fonts.heavy, size: 9, color: C.red2 });
        ctx.text(c.labelB, PT.marginX + colW + 3 * MM, y + (headH - 9) / 2, { font: fonts.heavy, size: 9, color: C.red2 });
        y += headH;
  ...
  function gapInfo(item) {
    return { pre: deent(item[0]), gap: deent(item[1]), post: deent(item[2]), deriv: deent(item[3]), hard: deent(item[4]) };
  }
  ...
  function cloneTask(t) {
    ...
    if (t.it) {
      o.it = {
        grund: deent(t.it.grund), deriv: deent(t.it.deriv), hard: deent(t.it.hard),
        gap: deent(t.it.gap), phen: t.it.phen, raw: t.it.raw
      };
    }
  ...
  global.LautePDF = { PT, buildWorksheetPDF };
